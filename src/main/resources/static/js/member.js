    ...
    .then((data) => {
      if (data.exists) {
        ...
        item.parentElement
          ...
          .setAttribute('hidden', '');
        window.location.href = `/member/emailJoinDetail?email=${encodeURIComponent(
          email.value.trim()
        )}`;
      }
    })
  ...
  if (otherCheckboxes) {
    otherCheckboxes.forEach((box) => {
      box.addEventListener('click', function (e) {
        ...
        // "전체 동의" 체크박스 상태 업데이트
        agreeAllCheckbox.querySelector('.checkbox_input').checked = [
          ...otherCheckboxes,
        ].every((v) => v.querySelector('input[type=checkbox]').checked);
...
function validateAndSignUp() {
  ...
  const isAgreeAllChecked = document.querySelectorAll(
    '.agree-list .checkbox_input'
  ).checked;

  const requiredCheckboxes = document.querySelectorAll(
    '.agree-list .checkbox_input[required]'
  );
  const isAllRequiredChecked = [...requiredCheckboxes].every(
