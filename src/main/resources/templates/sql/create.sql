DROP TABLE TAG_BOX CASCADE CONSTRAINTS;
...
(
    TAG_BOX_ID   NUMBER,               -- 태그 구분용
    BOARD_ID     NUMBER NOT NULL,      -- 게시판 구분용 카.
    TAG_NAME     VARCHAR2(20),         -- 사람들이 확인 가능한 테그 명
    TAG_PRIORITY NUMBER,               -- 태그 묶음인 경우, 해당 컬럼의 순서에 따라 출력.
    CREATE_DATE  DATE DEFAULT SYSDATE, -- 생성일
    POST_NUMBER  NUMBER NOT NULL,      -- 게시글 구분용 키.
    PRIMARY KEY (TAG_BOX_ID, BOARD_ID)
);
...
(
    ...
    SCRAP_POST_CODE NUMBER, -- 각 게시글 별 구분코드.
...
(
    ...
    COMMENT_POST_CODE NUMBER,                        -- 각 게시글 별 구분코드
...
DROP SEQUENCE SOLUTION_DETAIL_SEQ;
...
CREATE SEQUENCE SOLUTION_DETAIL_SEQ;
