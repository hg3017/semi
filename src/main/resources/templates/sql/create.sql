...
DROP TABLE COOK_DETAIL_STEP CASCADE CONSTRAINTS;
DROP TABLE COOK_DETAIL_INGREDIENT CASCADE CONSTRAINTS;
...
DROP TABLE GUIDE_LAB_CONN CASCADE CONSTRAINTS;
DROP TABLE GUIDE_SOLUTION_CONN CASCADE CONSTRAINTS;
...
CREATE TABLE COOK
(
	COOK_ID NUMBER UNIQUE,					 -- 요리해요 게시글 구분용
	BOARD_ID      NUMBER   NOT NULL, -- 각 게시판 별 구분코드.
	COOK_NAME      VARCHAR2(100),    -- 요리해요 레시피 게시글 제목.
	DESC_DETAIL   VARCHAR2(500),                 -- 요리연구소 레시피 게시글 내용.
	CREATER         NUMBER,                        -- 작성자. 맴버정보 연결용 키.
	CREATE_DATE   DATE DEFAULT SYSDATE NOT NULL, -- 생성일.
	MODIFY_DATE   DATE,                          -- 수정일.
  PRIMARY KEY (COOK_ID, BOARD_ID)
);


-- 요리해요 게시글의 재료설명에 대한 테이블입니다.
CREATE TABLE COOK_DETAIL_INGREDIENT
(
    COOK_DETAIL_ID         NUMBER PRIMARY KEY, -- 재료설명 테이블 구분용 키.
    COOK_ID                NUMBER,             -- 요리연구소 레시피 게시글 연결 키.
    COOK_PRIORITY          NUMBER,            -- 우선순위
    COOK_INGREDIENT_NAME   VARCHAR2(30),       -- 주재료, 부재료 이름 등
    COOK_INGREDIENT_DETAIL VARCHAR2(300),      -- 상세 재료(양배추, 달걀 등).
    SERVINGS              NUMBER,             -- 인분 수
    PREPARATION           NUMBER,             -- 준비시간
    COOKINGTIME           NUMBER              -- 조리시간
);
...
-- 요리해요 레시피 게시글의 스탭별 설명에 대한 테이블입니다.
CREATE TABLE COOK_DETAIL_STEP
(
    COOK_STEP_ID   	NUMBER,        -- 스탭별 설명 테이블 구분용 키.
    BOARD_ID      	NUMBER,        -- 이미지 정보 연결 키. IMG_LIST 테이블의 BOARD_ID 와 연결됩니다.
    COOK_ID        	NUMBER,        -- 요리연구소 레시피 게시글 연결 키.
    COOK_STEP      	NUMBER,        -- 스탭 단계
    COOK_STEP_NAME 	VARCHAR2(100), -- 스탭별 조리법 이름.
    COOK_STEP_DESC 	VARCHAR2(500), -- 스탭별 조리법 상세설명.
    COOK_STEP_TIP1 	VARCHAR2(500), -- 조리 방법 팁 1.
    COOK_STEP_TIP2 	VARCHAR2(500), -- 조리 방법 팁 2.
    COOK_STEP_TIP3 	VARCHAR2(500), -- 조리 방법 팁 3.
    COOK_STEP_TIP4 	VARCHAR2(500), -- 조리 방법 팁 4.
    PRIMARY KEY (COOK_STEP_ID, BOARD_ID)
);
...
CREATE TABLE GUIDE_LAB_CONN
(
    GUIDE_ID       NUMBER,
    LAB_ID         NUMBER,
    GUIDE_BOARD_ID NUMBER DEFAULT 10,
    LAB_BOARD_ID   NUMBER DEFAULT 20,
    PRIMARY KEY (GUIDE_ID, LAB_ID)
);

CREATE TABLE GUIDE_SOLUTION_CONN
(
    GUIDE_ID          NUMBER,
    SOLUTION_ID       NUMBER,
    GUIDE_BOARD_ID    NUMBER DEFAULT 10,
    SOLUTION_BOARD_ID NUMBER DEFAULT 30,
    PRIMARY KEY (GUIDE_ID, SOLUTION_ID)
);


DROP SEQUENCE EVENT_ID_SEQ;
DROP SEQUENCE COUNSELING_ID_SEQ;
DROP SEQUENCE COOK_STEP_SEQ;
DROP SEQUENCE COOK_DETAIL_SEQ;
...
CREATE SEQUENCE COOK_STEP_SEQ;
CREATE SEQUENCE COOK_DETAIL_SEQ;
...
ALTER TABLE "C##SEMI".COOK
    ADD CONSTRAINT COOK_MEMBER_FK FOREIGN KEY (CREATER) REFERENCES "C##SEMI"."MEMBER" (MEMBER_ID);

ALTER TABLE "C##SEMI".COOK_DETAIL_INGREDIENT
    ADD CONSTRAINT COOK_DETAIL_INGREDIENT_LAB_FK FOREIGN KEY (COOK_ID) REFERENCES "C##SEMI".COOK (COOK_ID);

ALTER TABLE "C##SEMI".COOK_DETAIL_STEP
    ADD CONSTRAINT COOK_DETAIL_STEP_LAB_FK FOREIGN KEY (COOK_ID) REFERENCES "C##SEMI".COOK (COOK_ID);



ALTER TABLE GUIDE_LAB_CONN ADD CONSTRAINT GUIDE_LAB_CONN_GUIDE_FK FOREIGN KEY (GUIDE_ID,GUIDE_BOARD_ID) REFERENCES GUIDE(GUIDE_ID,BOARD_ID);
ALTER TABLE GUIDE_LAB_CONN ADD CONSTRAINT GUIDE_LAB_CONN_LAB_FK FOREIGN KEY (LAB_ID,LAB_BOARD_ID) REFERENCES LAB(LAB_ID,BOARD_ID);

ALTER TABLE GUIDE_SOLUTION_CONN ADD CONSTRAINT GUIDE_SOLUTION_CONN_GUIDE_FK FOREIGN KEY (GUIDE_ID,GUIDE_BOARD_ID) REFERENCES GUIDE(GUIDE_ID,BOARD_ID);
ALTER TABLE GUIDE_SOLUTION_CONN ADD CONSTRAINT GUIDE_SOLUTION_CONN_SOLUTION_FK FOREIGN KEY (SOLUTION_ID,SOLUTION_BOARD_ID) REFERENCES SOLUTION(SOLUTION_ID,BOARD_ID);
